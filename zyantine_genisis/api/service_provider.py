...
class APIServiceProvider:
    ...
    def _initialize_services(self):
        ...
        self.logger.info(f"打印api_config={api_config}")
        if api_config.enabled and api_config.api_key:
            ...
            # 测试连接
            if openai_service.test_connection()[0]:
                ...
                self.logger.warning("OpenAI服务连接测试失败，将使用降级策略")
    ...
    def _initialize_components(self):
        ...
        # 创建降级策略
        self.fallback_strategy = FallbackStrategy(core_identity=CoreIdentity())
    ...
    def generate_reply(self, **kwargs) -> str:
        ...
        try:
            ...
            # 尝试降级
            try:
                fallback_reply = self.fallback_strategy.generate_fallback_reply(**kwargs)
                return fallback_reply
            except Exception as fallback_error:
                self.logger.error(f"降级策略也失败: {fallback_error}")
                return self._generate_emergency_response()

    def generate_reply_with_memory(self, **kwargs) -> str:
        """使用记忆生成回复"""
        # 如果有专门的记忆生成方法
        if hasattr(self.reply_generator, 'generate_reply_with_memory'):
            return self.reply_generator.generate_reply_with_memory(**kwargs)
        else:
            # 回退到普通生成
