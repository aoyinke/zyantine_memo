...
class FactAnchorProtocol:
    ...
    def __init__(self, memory_archives: ZyantineMemorySystem, api_service=None):
        """
        初始化事实锚定协议

        Args:
            memory_archives: 记忆系统
        ...
        if self.enable_api_verification:
            print("[事实锚定] API事实审查已启用")
        else:
            print("[事实锚定] API事实审查未启用，使用本地验证")
    ...
    def final_review(self, final_draft: str, context: Dict) -> Tuple[bool, str]:
        ...
        print(f"[事实锚定] 开始终审，需要审查 {len(statements)} 条陈述")
    ...
    def _api_based_final_review(self, final_draft: str, statements: List[str], context: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            print(f"[事实锚定] 调用API进行事实审查，陈述数: {len(statements)}")

            # 调用API进行事实审查
            verification_result, api_error = self.api_service.generate_reply(
                system_prompt=system_prompt,
                user_input=user_input,
                max_completion_tokens=500,
                temperature=0.3  # 使用低温以获得更确定性的事实判断
            )
            if api_error or not verification_result:
                print(f"[事实锚定] API审查失败: {api_error}, 回退到本地验证")
                return self._local_final_review(final_draft, statements, context)
