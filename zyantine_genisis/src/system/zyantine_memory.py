...
from ..memory.memory_store import HybridMemorySystem, EnhancedMemoryAlchemyEngine

# ============ 使用OpenAI嵌入的自衍体主系统 ============
class OpenAIEnhancedZyantineGenesisV2:
    """使用OpenAI嵌入增强的自衍体起源 V2.0"""
    ...
    def __init__(self, user_profile_data: Dict, self_profile_data: Dict,
        ...
        print(f"[系统] 初始化记忆系统，路径: {memory_config['storage_path']}")

        # 初始化OpenAI嵌入服务
        embedding_service = config.get_embedding_service()

        if not embedding_service:
            print("[系统] 警告：OpenAI嵌入服务不可用，将无法生成文本向量")
            print("[系统] 请检查API密钥配置或网络连接")

        # 初始化混合记忆系统（使用OpenAI嵌入）
        ...
        # 加载现有对话历史
        self.conversation_history = self.memory_system.get_conversation_history(
            limit=100, session_id=session_id
        )

        print(f"[系统] 加载了 {len(self.conversation_history)} 条历史对话")

        # 检查统计信息
        stats = self.memory_system.get_statistics(session_id)
        print(f"[系统] 现有记忆: {stats['faiss_memories']} 个向量记忆，"
              f"{stats['conversations']} 条对话")

        # 显示嵌入服务状态
        if embedding_service:
            embed_stats = embedding_service.get_statistics()
            print(f"[系统] 嵌入服务: {embed_stats['model']}，维度: {embed_stats['default_dimensions']}")

        # 初始化系统（保持原有架构）
        print("正在初始化自衍体 Genesis V2.0 (OpenAI嵌入增强版)...")
        ...
        # 使用增强的记忆引擎（使用OpenAI嵌入）
        self.memory_engine = EnhancedMemoryAlchemyEngine(
            user_profile_data, self_profile_data, self.memory_system, session_id
        )

        print("  ✓ 内在状态仪表盘校准")
        print("  ✓ 核心身份加载完成")
        print("  ✓ 增强记忆炼金术引擎启动 (使用OpenAI嵌入)")
        ...
        self.system_status = {
            ...
            "storage_path": memory_config["storage_path"],
            "memory_system": "faiss_with_openai_embedding",
            ...
            "components_loaded": 13,
            "memory_stats": stats
        }

        print("\n" + "=" * 50)
        print("自衍体 Genesis V2.0 (OpenAI嵌入增强版) 初始化完成")
        print(f"会话ID: {session_id}")
        print(f"嵌入模型: {openai_config['embedding_model']} ({openai_config['embedding_dimensions']}维)")
        print(f"聊天模型: {openai_config['chat_model'] if openai_config['enabled'] else '未启用'}")
        print("=" * 50 + "\n")
